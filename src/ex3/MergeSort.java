package ex3;
import global.Impressao;

public class MergeSort {


    public void string(String[] vetor) {


        int tamanho = vetor.length;
        if (tamanho <= 1) return;


        int indiceMeio = tamanho/2;


        String[] vetorEsquerdo = new String[indiceMeio];
        String[] vetorDireito = new String[tamanho-indiceMeio];


        int iTotal = 0;
        int iDireito = 0;


        for (; iTotal < tamanho; iTotal++) {


            if (iTotal < indiceMeio) {
                vetorEsquerdo[iTotal] = vetor[iTotal];
            } else {
                vetorDireito[iDireito] = vetor[iTotal];
                iDireito++;
            }
        }


        string(vetorEsquerdo);
        string(vetorDireito);
        mergeString(vetorEsquerdo,vetorDireito,vetor);


    }

    private void mergeString(String[] vetorEsquerdo, String[] vetorDireito, String[] vetor) {


        int tamanho = vetor.length;


        int tamanhoEsquerdo = tamanho/2;
        int tamanhoDireito = tamanho - tamanhoEsquerdo;
        int iTotal = 0, iEsquerda = 0, iDireito = 0;


        while (iEsquerda < tamanhoEsquerdo && iDireito < tamanhoDireito) {
            if(vetorEsquerdo[iEsquerda].compareTo(vetorDireito[iDireito]) < 0) {
                vetor[iTotal] = vetorEsquerdo[iEsquerda];
                iTotal++;
                iEsquerda++;
            } else {
                vetor[iTotal] = vetorDireito[iDireito];
                iTotal++;
                iDireito++;
            }
            Impressao.imprimirVetorString(vetor);
            System.out.println();
        }

        while(iEsquerda < tamanhoEsquerdo) {
            vetor[iTotal] = vetorEsquerdo[iEsquerda];
            iTotal++;
            iEsquerda++;
        }


        while(iDireito < tamanhoDireito) {
            vetor[iTotal] = vetorDireito[iDireito];
            iTotal++;
            iDireito++;
        }


    }


}